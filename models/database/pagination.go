package database

import (
	"math"

	paginator "github.com/pilagod/gorm-cursor-paginator"
	"gorm.io/gorm"
)

type (
	// RequestPaginatorParam struct for request with paginatioan
	RequestPaginatorParam struct {
		Page  int `json:"page"`
		Limit int `json:"limit"`
	}

	// PagingQuery is query for pagination
	PagingQuery struct {
		After  *string
		Before *string
		Limit  *int
		Order  *string
	}

	// Paginator struct response pagination
	Paginator struct {
		Records     interface{} `json:"records"`
		TotalRecord int64       `json:"total_record"`
		TotalPage   int         `json:"total_page"`
		Offset      int         `json:"offset"`
		Limit       int         `json:"limit"`
		Page        int         `json:"page"`
		PrevPage    int         `json:"prev_page"`
		NextPage    int         `json:"next_page"`
	}

	// PaginatorParam ...
	PaginatorParam struct {
		DB      *gorm.DB
		OrderBy []string
		ShowSQL bool
		Limit   int
		Page    int
	}

	// PaginationMeta ...
	PaginationMeta struct {
		After     int `json:"after"`
		Before    int `json:"before"`
		TotalPage int `json:"total"`
		Page      int `json:"page"`
	}
)

// GetModelPaginator ...
func GetModelPaginator(q PagingQuery) *paginator.Paginator {
	p := paginator.New()

	p.SetKeys("CreatedAt") // [default: "ID"] (supporting multiple keys, order of keys matters)

	if q.After != nil {
		p.SetAfterCursor(*q.After) // [default: nil]
	}

	if q.Before != nil {
		p.SetBeforeCursor(*q.Before) // [default: nil]
	}

	if q.Limit != nil {
		p.SetLimit(*q.Limit) // [default: 10]
	}

	if q.Order != nil && *q.Order == "asc" {
		p.SetOrder(paginator.ASC) // [default: paginator.DESC]
	}
	return p
}

// Paging ...
func Paging(p *PaginatorParam, result interface{}) *Paginator {
	db := p.DB

	if p.ShowSQL {
		db = db.Debug()
	}
	if p.Page < 1 {
		p.Page = 1
	}
	if p.Limit == 0 {
		p.Limit = 10
	}
	if len(p.OrderBy) > 0 {
		for _, o := range p.OrderBy {
			db = db.Order(o)
		}
	}

	done := make(chan bool, 1)
	var paginator Paginator
	var count int64
	var offset int

	go countRecords(db, result, done, &count)

	if p.Page == 1 {
		offset = 0
	} else {
		offset = (p.Page - 1) * p.Limit
	}

	db.Limit(p.Limit).Offset(offset).Find(result)
	<-done

	paginator.TotalRecord = count
	paginator.Records = result
	paginator.Page = p.Page

	paginator.Offset = offset
	paginator.Limit = p.Limit
	paginator.TotalPage = int(math.Ceil(float64(count) / float64(p.Limit)))

	if p.Page > 1 {
		paginator.PrevPage = p.Page - 1
	} else {
		paginator.PrevPage = p.Page
	}

	if p.Page == paginator.TotalPage {
		paginator.NextPage = p.Page
	} else {
		paginator.NextPage = p.Page + 1
	}
	return &paginator
}

func countRecords(db *gorm.DB, anyType interface{}, done chan bool, count *int64) {
	db.Model(anyType).Count(count)
	done <- true
}
